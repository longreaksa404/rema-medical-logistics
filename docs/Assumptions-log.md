...
| 70 | Delivery Bug Session | EMK3 and EMK2 are independent needs — a household with chronic illness AND a vulnerable member receives both, not EMK3 only | Section B and C are clear that kit types address different needs |
| 71 | Delivery Bug Session | POST /api/delivery/receipts accepts both kits[] array and legacy emkType+quantity — backwards compatible | Avoids breaking any existing calls while supporting multi-kit delivery |
| 72 | Pagination Session | Movement endpoints default to 20 per page, max 100 per request | 20 covers most operational views without overloading the response; 100 cap prevents accidental large queries |
| 73 | Pagination Session | Priority queue sorts all undelivered in memory before slicing — Prisma-level sort cannot replicate the Section C 4-level tiebreaker | ORDER BY in SQL cannot express the exact tiebreaker chain; in-memory sort is required for correctness |
| 74 | Pagination Session | Band filter in PriorityQueueTable operates on current page only after pagination added | Showing full-dataset band counts would require a separate aggregation query — per-page counts are acceptable for operational use |