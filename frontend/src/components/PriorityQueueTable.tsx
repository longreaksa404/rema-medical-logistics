...
const EMK_COLORS = { EMK1: 'text-accent-blue', EMK2: 'text-accent-green', EMK3: 'text-accent-red' } as const;

function SkeletonRow() {
  return (
    <tr className="border-b border-bg-border">
      {[...Array(6)].map((_, i) => (
        <td key={i} className="px-4 py-3"><div className="h-3 bg-bg-elevated rounded animate-pulse" /></td>
      ))}
    </tr>
  );
}
...
export const PriorityQueueTable = memo(function PriorityQueueTable({ districts }: PriorityQueueTableProps) {
  ...
  const bandCounts = useMemo(() => ({
    CRITICAL: households.filter((h: Household) => h.priorityBand === 'CRITICAL').length,
    HIGH:     households.filter((h: Household) => h.priorityBand === 'HIGH').length,
    MEDIUM:   households.filter((h: Household) => h.priorityBand === 'MEDIUM').length,
    STANDARD: households.filter((h: Household) => h.priorityBand === 'STANDARD').length,
  }), [households]);
        ...
        <table className="w-full">
          ...
          <tbody>
            {isLoading
              ...
              : filtered.map((h: Household) => {
                  const cfg = BAND_CONFIG[h.priorityBand];
                  return (
                    <tr key={h.id} className="border-b border-bg-border hover:bg-bg-elevated/50 transition-colors duration-75">
                        ...
                        <div className="flex items-center gap-1.5">
                          <div className="w-16 h-1.5 bg-bg-border rounded-full overflow-hidden">
                            <div className={`h-full rounded-full ${
                              h.totalScore >= 15 ? 'bg-accent-red' : h.totalScore >= 10 ? 'bg-accent-orange' : h.totalScore >= 5 ? 'bg-accent-yellow' : 'bg-accent-green'
                            }`} style={{ width: `${(h.totalScore / 20) * 100}%` }} />
                          </div>
                          <span className="font-mono text-xs text-text-secondary">{h.totalScore}/20</span>
                        </div>
                      </td>
                      <td className="px-4 py-2.5">
                        <span className={`font-mono text-xs ${
                          h.medicalUrgencyScore >= 8 ? 'text-accent-red font-semibold' : h.medicalUrgencyScore >= 5 ? 'text-accent-orange' : 'text-text-muted'
