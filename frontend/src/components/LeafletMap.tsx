...
// Real GeoJSON polygon boundaries for Phnom Penh districts
// (Dangkao R2259498 / Mean Chey R2259500 / Pou Senchey R9590423)
...
const MODE_FILL: Record<DeliveryMode, string> = {
  MOTORBIKE:       '#3fb950',
  BICYCLE_OR_FOOT: '#d29922',
  BOAT:            '#f0883e',
  SUSPENDED:       '#f85149',
};

function avgDepthColor(depths: Record<string, number>): string {
  const vals = Object.values(depths);
  if (vals.length === 0) return MODE_FILL.MOTORBIKE;
  const avg = vals.reduce((a, b) => a + b, 0) / vals.length;
  return MODE_FILL[depthToMode(avg)];
}
...
export function LeafletMap({
  ...
  useEffect(() => {
    ...
    // Phnom Penh centre — auto-fits to all 3 districts
    ...
    // Draw each district polygon from real GeoJSON
    districts.forEach((district) => {
      const geoKey = district.name; // must match key in DISTRICT_GEOJSON
      const geo = DISTRICT_GEOJSON[geoKey];
      if (!geo) return;

      const depths = zoneDepths[district.name] ?? {};
      const fillColor = avgDepthColor(depths);
      ...
      // Hover highlight
      layer.on('mouseover', () => {
        layer.setStyle({ fillOpacity: 0.55, weight: 3.5 });
      });
      layer.on('mouseout', () => {
      ...
      // District name tooltip (permanent label on polygon)
      layer.bindTooltip(district.name, {
        permanent: true,
        direction: 'center',
        className: 'rema-district-label',
      });

      layer.addTo(map);
      layersRef.current[district.districtId] = layer;
    });

    // Fit map to all district polygons
    const allCoords: [number, number][] = [];
    districts.forEach((d) => {
      const geo = DISTRICT_GEOJSON[d.name];
      if (geo) {
        geo.coordinates[0].forEach(([lng, lat]) => allCoords.push([lat, lng]));
      }
    });
    ...
    if (!document.getElementById('rema-leaflet-styles')) {
      ...
      style.textContent = `
        ...
        .rema-district-label::before { display: none; }
        .leaflet-tooltip-top.rema-district-label::before { display: none; }
      `;
      document.head.appendChild(style);
    }

    // Cleanup on unmount — prevents sign-out freeze
    return () => {
      map.remove();
      mapRef.current = null;
      layersRef.current = {};
  ...
  // ── Update fill colors when water depths change ──────────────────────────────
  useEffect(() => {
    const L = (window as any).L;
    if (!L || !mapRef.current) return;

    districts.forEach((district) => {
      const layer = layersRef.current[district.districtId];
      if (!layer) return;
  ...
  useEffect(() => {
    ...
    Object.entries(layersRef.current).forEach(([id, layer]) => {
      ...
      const depths = zoneDepths[district.name] ?? {};
      const fillColor = avgDepthColor(depths);
      layer.setStyle({ fillOpacity: 0.35, weight: 2.5, color: fillColor, fillColor });
    });

    // Highlight selected
    layersRef.current.__selected = selectedDistrictId;
    if (selectedDistrictId) {
      const layer = layersRef.current[selectedDistrictId];
      if (layer?.setStyle) {
        const district = districts.find((d) => d.districtId === selectedDistrictId);
        if (district) {
          const depths = zoneDepths[district.name] ?? {};
          const fillColor = avgDepthColor(depths);
          layer.setStyle({ fillOpacity: 0.6, weight: 4, color: '#ffffff', fillColor });
        }
      }
    }
