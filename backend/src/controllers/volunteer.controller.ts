...
export async function create(req: Request, res: Response): Promise<void> {
  const { districtId, name, phone, role } = req.body;
  ...
  try {
    const volunteer = await createVolunteer({ districtId, name, phone, role });
...
export async function update(req: Request, res: Response): Promise<void> {
  ...
  try {
    const volunteer = await updateVolunteer(req.params.id, { name, phone, status, role });
    res.json(volunteer);
  } catch (err) {
